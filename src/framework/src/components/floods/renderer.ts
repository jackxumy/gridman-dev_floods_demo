...
export default class FloodsRenderer {
    ...
    async initScene() {
        ...
        const { center4326: sceneCenter } = this.calculateUnifiedNormalizationAndCenter();
        ...
        const scope = this;
        // 使用计算得到的中心点
        const { center4326: modelCenter } = this.calculateUnifiedNormalizationAndCenter();
    ...
    async loadInitialDataAndUpdateConfig() {
        ...
        try {
            ...
            // 获取初始的水体数据（不开启轮询，只获取第一批数据）
            this._floodsResources.setDynamicPlaybackMode(false); // 暂时设置为非动态模式
            ...
            if (success) {
                // 再获取一次以确保有足够数据创建快照
                await (this._floodsResources as any).fetchWaterDataStep();
                
                // 创建快照
                await (this._floodsResources as any).createSnapshot();
    ...
    startContinuousWaterPolling() {
        ...
        // 启用动态播放模式
        this._floodsResources.setDynamicPlaybackMode(true, 1000); // 每帧1秒
    ...
    async updateWaterResources() {
        ...
        try {
            ...
            // 动态播放模式：增量更新，避免重新赋值整个 _waterData
            if (this._floodsResources.getDynamicPlaybackMode() && this._waterData) {
                // 只更新变化的部分，保持现有的纹理引用稳定
                const previousTextureCount = this._waterData.waterTextures?.length || 0;
                const newTextureCount = data.waterTextures?.length || 0;
                
                if (newTextureCount > previousTextureCount) {
                    // 只添加新的纹理，不替换现有的
                    if (!this._waterData.waterTextures) {
                        this._waterData.waterTextures = [];
                    }
                    
                    // 添加新纹理
                    for (let i = previousTextureCount; i < newTextureCount; i++) {
                        if (data.waterTextures && data.waterTextures[i]) {
                            this._waterData.waterTextures.push(data.waterTextures[i]);
                        }
                    }
                    
                    // 更新数组数据（增量更新）
                    this._waterData.waterHuvMaps = [...data.waterHuvMaps];
                    this._waterData.waterHeightMin = [...data.waterHeightMin];
                    this._waterData.waterHeightMax = [...data.waterHeightMax];
                    this._waterData.velocityUMin = [...data.velocityUMin];
                    this._waterData.velocityUMax = [...data.velocityUMax];
                    this._waterData.velocityVMin = [...data.velocityVMin];
                    this._waterData.velocityVMax = [...data.velocityVMax];
                    
                    console.log(`Dynamic mode: incremental update, texture count: ${previousTextureCount} -> ${this._waterData.waterTextures.length}`);
                }
                
                return; // 动态模式直接返回，避免重新赋值
            }
            ...
            // 如果数据已经包含预加载的纹理，直接使用
            if (this._waterData.waterTextures && this._waterData.waterTextures.length > 0) {
                console.log('Using pre-loaded water textures from snapshot');
            } else {
                // 否则手动加载纹理
                console.log('Loading water textures manually...');
                const textureLoader = this._textureLoader;
            ...
            console.log('Water resources updated with new data');
        } catch (error) {
    ...
    updateWaterUniforms(time: number) {
        ...
        // 动态播放模式的特殊处理
        const isDynamicMode = this._floodsResources.getDynamicPlaybackMode();
        ...
        if (isDynamicMode) {
            // 动态模式：使用固定的帧时长
            const frameDuration = this._floodsResources.getFrameDuration(); // 转换为秒
            currIndex = Math.floor(time / frameDuration) % numRasters;
            nextIndex = (currIndex + 1) % numRasters;
            timeStep = (time % frameDuration) / frameDuration;
            ...
            currIndex = Math.floor(time / durationTime) % numRasters;
            nextIndex = (currIndex + 1) % numRasters;
            timeStep = (time % durationTime) / durationTime;
        }
        ...
        // 确保纹理存在才设置
        if (this._waterData.waterTextures[currIndex]) {
            uniforms.huvMapBefore.value = this._waterData.waterTextures[currIndex];
        }
        if (this._waterData.waterTextures[nextIndex]) {
